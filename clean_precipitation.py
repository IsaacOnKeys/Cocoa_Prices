...
def clean_and_convert(element):
    ...
    try:
        ...
        date_str = None  # Will be handled in validation
...
# Function to format the output as CSV for valid records
...
# DoFn for Validation and Conversion
class ValidateAndConvert(beam.DoFn):
    ...
    def process(self, element):
        ...
        # 1. Schema Validation: Check presence of 'date', 'PRECTOTCORR', 'GWETROOT'
        ...
        # 5. Uniqueness Checks
        # Note: Uniqueness is handled separately using GroupByKey
        ...
        # Already handled by date creation
        
        # 8. Format Validation
        # Ensured during CSV formatting
        
        # 9. Null and Default Value Handling
        # Handled by setting None and checking above
...
# Function for uniqueness checks using GroupByKey
...
def run():
    ...
    pipeline_options.view_as(StandardOptions).runner = "DirectRunner"  # Use 'DataflowRunner' for cloud execution
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        # Step 3: Handle Uniqueness Checks
        unique_records = (
            validated_records.valid
            | "Key by Date" >> beam.Map(key_by_date)
            | "Group by Date" >> beam.GroupByKey()
            | "Filter Unique Dates" >> beam.ParDo(filter_unique_dates).with_outputs('invalid', main='valid_unique')
        )
        
        # Step 4: Write Valid Records to CSV
        (
        ...
        # Step 5: Write Invalid Records to Separate CSV
        (
